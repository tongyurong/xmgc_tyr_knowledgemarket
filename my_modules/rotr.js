...
_rotr.apis.allstudy= function() {
    var ctx = this;
    var co = $co(function * () {
            var sqlstr="select * from (select * from CourseDetail) c,(select CourseId,count(*) as count from BuyRecord group by CourseId order by count desc) as b where c.CourseID=b.CourseId order by b.count desc";
...
_rotr.apis.personCenterList1 = function(){
    ...
    var co = $co(function * () {
        ...
        var collectsql="select * from Collection,CourseDetail where Collection.State=1 and CourseDetail.CourseID=Collection.CourseId and UserId=("+userId+")";
...
_rotr.apis.personCenterList11 = function(){
    var ctx = this;
    var co = $co(function * () {
        var userId=yield _fns.getUidByCtx(ctx);
        var cancel1 = ctx.query.cancel1 || ctx.request.body.cancel1;
        var collectsql="delete from Collection where UserId="+userId+" and CourseId="+cancel1;
        var writelist=yield _ctnu([_mysql.conn,'query'],collectsql);
        ctx.body = writelist;
        return ctx;
    });
...
_rotr.apis.select = function(){
    ...
    var co = $co(function * () {
        ...
        var pageindex = ctx.query.pageindex  || ctx.request.body.pageindex;
        var data2 = ctx.query.data2 || ctx.request.body.data2;
        if(!pageindex){
            pageindex=1;
        }
        if(!data2){
        ...
        switch(data2){
            case '0':var collectsql="call fenye1('"+data+"','"+pageindex+"');";
                break;
            case '1':var collectsql="call fenyehigh('"+data+"','"+pageindex+"');";
                break;
            case '2': var collectsql="call fenyelow('"+data+"','"+pageindex+"');";
                break;
            case '3':
                var collectsql="call fenyelittle('"+data+"','"+pageindex+"');";
                break;
            case '4':var collectsql="call fenyemore('"+data+"','"+pageindex+"');";
                break;
            default : console.log("default");
        }
        var writelist=yield _ctnu([_mysql.conn,'query'],collectsql);
        writelist.push(pageindex)
        ctx.body = writelist;
        return ctx;
    });
    return co;
...
_rotr.apis.detailcollect= function() {
    ...
    var co = $co(function * () {
        var userId=yield _fns.getUidByCtx(ctx);
        var courseId=ctx.query.courseid||ctx.request.body.courseid;
        var sqlstr = "select UserId,CourseId,State from Collection where  UserId="+userId+" and CourseId='"+courseId+"'";
        var rows = yield _ctnu([_mysql.conn, 'query'], sqlstr);
        if (rows.length==0){
        var sqlstr2 = "insert Collection (CourseId,UserId,State) values("+courseId +","+userId+",1)";
...
_rotr.apis.detailbuy= function() {
    ...
    var co = $co(function * () {
        ...
        var course=courseId.split(',')
        console.log(course)
        for(var i=0;i<course.length;i++){
            console.log(">>>test",course[i])
            var sqlstr = "select UserId,CourseId from BuyRecord where UserId="+userId+" and CourseId='"+course[i]+"'";
            console.log(sqlstr)
            var rows = yield _ctnu([_mysql.conn, 'query'], sqlstr);
            console.log(">>>rows",rows)
            if (rows.length==0){
                var sqlstr2 = "insert BuyRecord (CourseId,UserId,ProductState) values("+course[i] +","+userId+",0)";
                var rows1 = yield _ctnu([_mysql.conn, 'query'], sqlstr2);
            }
        }
...
_rotr.apis.passapply = function(){
    ...
    var co = $co(function * () {
        ...
        var tablecourseclassifydetai=ctx.query.tablecourseclassifydetai||ctx.request.body.tablecourseclassifydetai;
        ...
        var rows=yield _ctnu([_mysql.conn,'query'],sqlstr);
        var sqlstr1="insert CourseDetail(CourseImgURL,CourseName,CoursePrice,CourseTimes,Courseclassify,CourseclassifyDetail,Coursebrief) values('"+tableimg+"','"+tablecoursename+"','"+tableprice+"','"+tabletimes+"','"+tablecourseclassify+"','"+tablecourseclassifydetai+"','"+tablebrief+"')";
        var rows1=yield _ctnu([_mysql.conn,'query'],sqlstr1);
        ...
        var rows3=yield _ctnu([_mysql.conn,'query'],sqlstr3);
          var ID=rows3; console.log(">>>rows3",ID[0].CourseID);
        for(var i=1;i<Time.length;i++){
            var sqlstr4="insert CourseEach(CourseID,CourseEachName,CourseEachTime,CourseEachURL) values('"+ID[0].CourseID+"','"+Name[i]+"','"+Time[i]+"','"+Url[i]+"')";
...
//判断是否加入购物车
_rotr.apis.shopcar= function() {
    var ctx = this;
    var co = $co(function * () {
        var userId=yield _fns.getUidByCtx(ctx);
        var spid=ctx.query.spid||ctx.request.body.spid;
        var sqlstr = "select * from BuyRecord where UserId="+userId+" and CourseId='"+spid+"'";
        var rows = yield _ctnu([_mysql.conn, 'query'], sqlstr);
        console.log(rows);
        ctx.body = rows;
        return ctx;
